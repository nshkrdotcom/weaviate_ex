# Multi-Tenancy Example
# Run: mix run examples/06_tenants.exs

unless Code.ensure_loaded?(WeaviateEx) do
  Mix.install([{:weaviate_ex, path: "."}])
end

Code.require_file("example_helper.exs", __DIR__)

alias WeaviateEx.API.{Collections, Tenants, VectorConfig}

ExampleHelper.check_weaviate!()

ExampleHelper.section("Multi-Tenancy - Tenant Management")

client = ExampleHelper.client!()
class_name = ExampleHelper.unique_class("TenantArticle")
ExampleHelper.reset_collection!(client, class_name)

# Create multi-tenant collection
config =
  VectorConfig.new(class_name)
  |> VectorConfig.with_vectorizer(:none)
  |> VectorConfig.with_multi_tenancy(enabled: true)
  |> VectorConfig.with_properties([
    %{"name" => "title", "dataType" => ["text"]}
  ])

Collections.create(client, config)

# Create tenants
ExampleHelper.step("Create multiple tenants")

ExampleHelper.command(
  ~s/Tenants.create(client, "#{class_name}", ["CompanyA", "CompanyB", "CompanyC"])/
)

{:ok, created} = Tenants.create(client, class_name, ["CompanyA", "CompanyB", "CompanyC"])
ExampleHelper.result("Created Tenants", Enum.map(created, & &1["name"]))

# List tenants
ExampleHelper.step("List all tenants")
ExampleHelper.command(~s/Tenants.list(client, "#{class_name}")/)
{:ok, tenants} = Tenants.list(client, class_name)
ExampleHelper.success("Found #{length(tenants)} tenants")

# Get specific tenant
ExampleHelper.step("Get tenant details")
ExampleHelper.command(~s/Tenants.get(client, "#{class_name}", "CompanyA")/)
{:ok, tenant} = Tenants.get(client, class_name, "CompanyA")
ExampleHelper.result("Tenant", tenant)

# Check existence
ExampleHelper.step("Check if tenant exists")
ExampleHelper.command(~s/Tenants.exists?(client, "#{class_name}", "CompanyA")/)
{:ok, true} = Tenants.exists?(client, class_name, "CompanyA")
ExampleHelper.success("Tenant exists")

# Deactivate tenant (set to COLD)
ExampleHelper.step("Deactivate tenant (set to COLD)")
ExampleHelper.command(~s/Tenants.deactivate(client, "#{class_name}", "CompanyB")/)
{:ok, _} = Tenants.deactivate(client, class_name, "CompanyB")
ExampleHelper.success("Tenant CompanyB deactivated")

# List active tenants only
ExampleHelper.step("List only active tenants")
ExampleHelper.command(~s/Tenants.list_active(client, "#{class_name}")/)
{:ok, active} = Tenants.list_active(client, class_name)
ExampleHelper.result("Active Tenants", Enum.map(active, & &1["name"]))

# Activate tenant
ExampleHelper.step("Activate tenant (set to HOT)")
ExampleHelper.command(~s/Tenants.activate(client, "#{class_name}", "CompanyB")/)
{:ok, _} = Tenants.activate(client, class_name, "CompanyB")
ExampleHelper.success("Tenant CompanyB activated")

# Count tenants
ExampleHelper.step("Count total tenants")
ExampleHelper.command(~s/Tenants.count(client, "#{class_name}")/)
{:ok, count} = Tenants.count(client, class_name)
ExampleHelper.success("Total tenants: #{count}")

# Delete tenant
ExampleHelper.step("Delete tenant")
ExampleHelper.command(~s/Tenants.delete(client, "#{class_name}", "CompanyC")/)
{:ok, _} = Tenants.delete(client, class_name, "CompanyC")
ExampleHelper.success("Deleted CompanyC")

ExampleHelper.cleanup(client, class_name)
IO.puts("\n#{ExampleHelper.green("✓")} Example complete!\n")
